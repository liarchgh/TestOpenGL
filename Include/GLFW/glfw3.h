 ...
 * (Note: For Cygwin the compiler flag -m_WIN32 should be used, but to
 * make sure that things run smoothly for Cygwin users, we add __CYGWIN__
 * to the list of "valid _WIN32 identifiers", which removes the need for
 * -m_WIN32)
 */
#if !defined(_WIN32) && (defined(__WIN32__) || defined(_WIN32) || defined(__CYGWIN__))
 #define _WIN32
#endif /* _WIN32 */

/* In order for extension support to be portable, we need to define an
 * OpenGL function call method. We use the keyword APIENTRY, which is
 * defined for _WIN32. (Note: Windows also needs this for <GL/gl.h>)
 */
#ifndef APIENTRY
 #ifdef _WIN32
 ...
 */
 ...
 /* We are building a _WIN32 DLL */
 #define GLFWAPI __declspec(dllexport)

#elif defined(_WIN32) && defined(GLFW_DLL)

 /* We are calling a _WIN32 DLL */
 #if defined(__LCC__)
 ...
 /* We are either building/calling a static lib or we are non-_WIN32 */
 #define GLFWAPI
 ...
 *
 *  For example, when compiling GLFW 3.0 with MinGW using the _WIN32 and WGL
 *  back ends, the version string may look something like this:
 *
 *      3.0.0 _WIN32 WGL MinGW
 *
